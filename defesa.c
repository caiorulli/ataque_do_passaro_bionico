...
void defesasAtiram(unsigned int timestep) {
   ...
   for (q = listaDefesa->prox; q != NULL; q = q->prox) {
      if (!(timestep % q->firerate) && distancia(q->posicao, nave->posicao) <= q->range) {
...
void desenhaDefesas(void) {
   ...
   for (p = listaDefesa->prox; p != NULL; p = p->prox) {
      ...
      glBegin(GL_QUADS);
         ...
         glVertex3f(p->posicao.x - 2 + 5 * ((double) p->hp / DEFESAHP), p->posicao.y + 3, p->posicao.z + 0.01);
         glVertex3f(p->posicao.x - 2 + 5 * ((double) p->hp / DEFESAHP), p->posicao.y + 4, p->posicao.z + 0.01);
      glEnd();
   }
}

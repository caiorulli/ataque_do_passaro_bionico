...
void atualizaNave() {
  ...
  if (nave->posicao.x < LIMITEESQ)
    nave->posicao.x = LIMITEESQ;
  if (nave->posicao.x > LIMITEDIR)
    nave->posicao.x = LIMITEDIR;
  if (nave->posicao.y < LIMITEINF + 1.7)
    nave->posicao.y = LIMITEINF + 1.7;
  if (nave->posicao.y > LIMITESUP)
...
Vetor posicaoDaNave() { return nave->posicao; }
...
void desenhaNave(void) {
  ...
  glVertex3f(-1, +0.5, -1);
  glVertex3f(-1, +0.5, +1);
  glVertex3f(-modulo(cos((double)timestep / 25)) * 2 - 1,
             sin((double)timestep / 25) * 2 + 0.5, +0.5);
